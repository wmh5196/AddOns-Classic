...
local tUnit;
local tImpact;
function VUHDO_parseCombatLogEvent(aMsg, aDstGUID, aMsg1, aMsg2, aMsg4, aSourceGUID, aSourceFlags)
	...
	if tImpact ~= 0 then
		VUHDO_addUnitHealth(tUnit, tImpact, aSourceGUID);
		if tUnit == sCurrentTarget then	VUHDO_addUnitHealth("target", tImpact, aSourceGUID); end
		if tUnit == sCurrentFocus then VUHDO_addUnitHealth("focus", tImpact, aSourceGUID); end
	end
end
